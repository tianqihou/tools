...
namespace myDLL
{
    public class ConfigParser
    {
         ...
         * 例如 GetPrivateProfileSectionNames声明为CharSet.Auto,
		 * 那么就应该使用 Marshal.PtrToStringAuto来读取相关内容 
		 * 
        ...
        string lpKeyName, string lpDefault, [In, Out] char[] lpReturnedString, uint nSize, string lpFileName);
        ...
        [DllImport("kernel32.dll", CharSet = CharSet.Auto)]
        private static extern uint GetPrivateProfileString(string lpAppName,
        string lpKeyName, string lpDefault, string lpReturnedString, uint nSize, string lpFileName);
		...
		public void Load(String iniFile)
		{
			if (System.IO.Path.IsPathRooted(iniFile))
				...
				this.iniFile = AppDomain.CurrentDomain.BaseDirectory + iniFile;
		}
		/// <summary>  
        ...
        public string[] Items(string section)
        {
